...
from .config import get_settings, ENV_FILE
from .lab import LabInvestigationRequest, build_lab_presets, run_lab_investigation

load_dotenv(dotenv_path=ENV_FILE)
settings = get_settings()

# Configuration
_go_server_path = Path(settings.go_server_path)
GO_SERVER_PATH = str(
    _go_server_path if _go_server_path.is_absolute() else (ENV_FILE.parent / _go_server_path).resolve()
)
...
def resolve_cluster_id(explicit_cluster_id: Optional[int], db: Session, conversation_id: Optional[int] = None) -> int:
    if explicit_cluster_id:
        return explicit_cluster_id

    conversation = db.get(models.Conversation, conversation_id) if conversation_id else None
    if conversation and conversation.cluster_id:
        return conversation.cluster_id

    first_cluster = db.exec(select(models.Cluster)).first()
    if not first_cluster:
        raise HTTPException(status_code=503, detail="No clusters configured.")
    return first_cluster.id
...
async def list_lab_presets():
    return {
        ...
        "presets": build_lab_presets(),
    }

@app.post("/api/lab/investigations")
async def create_lab_investigation(request: LabInvestigationRequest, db: Session = Depends(get_session)):
    cluster_id = resolve_cluster_id(request.clusterId, db)
    mcp_session = await session_manager.get_mcp_session(cluster_id, db)
    try:
        return await run_lab_investigation(
            client=client,
        ...
        raise HTTPException(
            status_code=502,
            detail=f"Lab investigation failed: {exc}"
        ) from exc
...
async def chat(request: ChatRequest, db: Session = Depends(get_session)):
    ...
    cluster_id = resolve_cluster_id(request.cluster_id, db, request.conversation_id)
    ...
    read_tools = []
    write_tools = []
    READ_PREFIXES = ["get_", "list_", "describe_", "explain_", "query_", "probe_", "check_", "ssh_ls", "ssh_cat", "ssh_tail", "ssh_grep", "ssh_ps", "ssh_df", "ssh_free", "ssh_netstat"]
