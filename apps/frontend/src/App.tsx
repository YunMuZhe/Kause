...
const App: React.FC = () => {
  const [viewMode, setViewMode] = useState<'chat' | 'lab'>('lab');
      ...
      <main className="flex-1 flex flex-col min-w-0 bg-gradient-to-b from-[#0d1117] to-[#0a0c10]">
        <header className="h-14 border-b border-[#30363d] flex items-center justify-between px-6 bg-[#0d1117]/80 backdrop-blur-md sticky top-0 z-10">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-sm font-semibold tracking-tight uppercase opacity-90">
              {viewMode === 'lab' ? 'Fault Lab 2.0' : (currentConversationId ? 'Conversation Session' : 'K8s Cluster Copilot')}
            </h1>
            <div className="ml-4 hidden items-center gap-1 rounded-xl border border-[#30363d] bg-[#161b22] p-1 sm:flex">
              <button
                type="button"
                onClick={() => setViewMode('lab')}
                className={`rounded-lg px-3 py-1.5 text-xs font-semibold transition ${viewMode === 'lab' ? 'bg-blue-500/20 text-blue-300' : 'text-[#8b949e] hover:text-white'}`}
              >
                Lab
              </button>
              <button
                ...
                className={`rounded-lg px-3 py-1.5 text-xs font-semibold transition ${viewMode === 'chat' ? 'bg-blue-500/20 text-blue-300' : 'text-[#8b949e] hover:text-white'}`}
              >
                Chat
              </button>
            </div>
        ...
        {viewMode === 'lab' ? (
          <div className="flex-1 overflow-auto">
            <LabWorkbench currentClusterId={currentClusterId} />
          </div>
        ) : (
